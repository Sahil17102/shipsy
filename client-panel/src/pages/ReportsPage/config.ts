...
/** Map of field key → label */
export const FIELD_LABEL_MAP: Record<string, string> = Object.fromEntries(
  FIELD_CATEGORIES.flatMap((cat) => cat.fields.map((f) => [f.key, f.label])),
);
...
export const REPORT_PRESETS: ReportPreset[] = [
  {
    ...
    description: "Order, status, courier, AWB, and delivery info",
    keys: [
      "orderId", "orderDate", "status", "awb", "serviceProvider",
      "customerName", "deliveryCity", "deliveryState", "deliveryPincode",
      "paymentType", "shippedAt", "deliveredAt",
    ],
  },
  {
    id: "financial_report",
    label: "Financial Report",
    description: "All charges, amounts, and billing details",
    keys: [
  ...
  },
  {
    ...
    description: "Track delivery times and outcomes",
    keys: [
