...
import { Button, InputNumber, Input, Segmented, Tag, Tooltip, Switch } from "antd";
import {
  Package,
  Truck,
  Zap,
...
import {
  ...
  useB2bAvailableCouriers,
  type AvailableCourier,
  type B2bAvailableCourier,
} from "@/queries/useDelhiveryRate";
...
const emptyPackage: B2bPackageState = {
  quantity: "1",
  weight: "",
  length: "",
  breadth: "",
...
export function RateCalculatorPage() {
  ...
  const [b2cForm, setB2cForm] = useState<B2cFormState>(initialB2c);
  const [b2bForm, setB2bForm] = useState<B2bFormState>(initialB2b);

  const b2cQuery = useAvailableCouriers();
  const b2bQuery = useB2bAvailableCouriers();

  function handleReset() {
    setB2cForm(initialB2c);
    setB2bForm(initialB2b);
    b2cQuery.reset();
    b2bQuery.reset();
  }
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Calculate shipping rates across all available couriers"
      />
...
interface B2cCalculatorProps {
  form: B2cFormState;
  setForm: React.Dispatch<React.SetStateAction<B2cFormState>>;
  query: ReturnType<typeof useAvailableCouriers>;
  onReset: () => void;
}

function B2cCalculator({ form, setForm, query, onReset }: B2cCalculatorProps) {
  const { mutate, data: couriers, isPending } = query;
    ...
    <>
      ...
      <div className="bg-background-elevated border border-border rounded-xl p-5 mb-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-foreground mb-1.5">
              <Weight className="inline w-3.5 h-3.5 mr-1" />
              Weight (grams)
            </label>
            <InputNumber
              className="!w-full"
          ...
          <div>
            ...
            <div className="flex gap-2">
              <InputNumber
                ...
                placeholder="L"
                min={0}
                value={form.length ? Number(form.length) : undefined}
                onChange={(v) => set("length", v ? String(v) : "")}
              />
              <InputNumber
                className="!w-full"
                placeholder="B"
                min={0}
                value={form.breadth ? Number(form.breadth) : undefined}
                onChange={(v) => set("breadth", v ? String(v) : "")}
              />
              <InputNumber
                className="!w-full"
                placeholder="H"
                min={0}
                value={form.height ? Number(form.height) : undefined}
                onChange={(v) => set("height", v ? String(v) : "")}
              />
            </div>
          </div>
      ...
      {couriers && couriers.length === 0 && (
        <EmptyState description="No couriers are available for this route and parameters." />
      )}
      ...
      {couriers && couriers.length > 0 && (
        <div>
          <p className="text-sm text-muted mb-4">
            ...
            Zone: <strong>{couriers[0].zone.name}</strong> ({couriers[0].zone.code})
          </p>
...
function B2cCourierCard({ courier }: { courier: AvailableCourier }) {
  return (
    <div className="bg-background-elevated border border-border rounded-xl p-5 flex flex-col sm:flex-row sm:items-center gap-4 hover:border-primary/30 transition-colors">
      <div className="flex items-center gap-3 flex-1 min-w-0">
        {courier.logo ? (
          <img src={courier.logo} alt={courier.name} className="w-10 h-10 rounded-lg object-contain bg-background p-1 border border-border-light" />
        ) : (
          <div className="w-10 h-10 rounded-lg bg-background border border-border-light flex items-center justify-center">
            <Truck className="w-5 h-5 text-muted" />
          </div>
        )}
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <span className="font-semibold text-foreground truncate">{courier.name}</span>
            {courier.tag === "economy" && (
              <Tag color="green" className="!text-xs">Cheapest</Tag>
            )}
            {courier.tag === "fastest" && (
              <Tag color="blue" className="!text-xs">
                <Zap className="inline w-3 h-3 mr-0.5" />Fastest
              </Tag>
            )}
          </div>
...
interface B2bCalculatorProps {
  form: B2bFormState;
  setForm: React.Dispatch<React.SetStateAction<B2bFormState>>;
  query: ReturnType<typeof useB2bAvailableCouriers>;
  onReset: () => void;
}

function B2bCalculator({ form, setForm, query, onReset }: B2bCalculatorProps) {
  const { mutate, data: couriers, isPending } = query;
  ...
  // Weight summary (factors in quantity per box)
  const totals = {
  ...
  return (
    <>
      ...
      {/* Results */}
      {couriers && couriers.length === 0 && (
        <EmptyState description="No B2B couriers are available for this route and parameters." />
      )}
      ...
      {couriers && couriers.length > 0 && (
        <div>
          <p className="text-sm text-muted mb-4">
            {couriers.length} courier{couriers.length > 1 ? "s" : ""} available &middot;
            Zone:{" "}
            <strong>{couriers[0].zone.originName}</strong> ({couriers[0].zone.originCode}) →{" "}
            <strong>{couriers[0].zone.destinationName}</strong> ({couriers[0].zone.destinationCode})
          </p>
          <div className="grid gap-4">
            {couriers.map((c) => (
...
function B2bCourierCard({ courier }: { courier: B2bAvailableCourier }) {
  return (
    <div className="bg-background-elevated border border-border rounded-xl p-5 hover:border-primary/30 transition-colors">
      {/* Header */}
        ...
        <div className="flex items-center gap-3 flex-1 min-w-0">
          {courier.logo ? (
            <img
              src={courier.logo}
              alt={courier.name}
              className="w-10 h-10 rounded-lg object-contain bg-background p-1 border border-border-light"
            />
          ) : (
            <div className="w-10 h-10 rounded-lg bg-background border border-border-light flex items-center justify-center">
              <Truck className="w-5 h-5 text-muted" />
