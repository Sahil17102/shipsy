...
interface ResponsiveTableProps<T> extends Omit<TableProps<T>, "columns"> {
  ...
  /** URL builder for mobile card tap-to-navigate. When provided, the whole card becomes tappable with a chevron indicator. */
  mobileHref?: (record: T) => string;
}
...
function MobileCard<T extends Record<string, any>>({
  ...
  const onRowClick = rowProps?.onClick as ((e: React.MouseEvent) => void) | undefined;

  // href takes priority over onRow click for navigation
  const isTappable = !!(href || onRowClick);
  ...
  return (
    ...
    >
      {/* ── Card header ── */}
      <div
        className={`flex items-center gap-2 px-3.5 py-2.5 bg-surface-muted/40 border-b border-border-light/60 ${isTappable ? "cursor-pointer active:bg-primary-bg" : ""}`}
      ...
      >
        ...
        {/* Chevron indicator when card is tappable and no menu present */}
        {isTappable && !hasMenu && (
      ...
      {bodyColumns.length > 0 && (
        <div
          className={`px-3.5 py-2.5 grid grid-cols-2 gap-x-4 gap-y-2.5 ${isTappable ? "cursor-pointer active:bg-primary-bg" : ""}`}
          onClick={isTappable ? handleTap : undefined}
        >
          {bodyColumns.map((col) => {
            ...
            return (
              ...
              >
                {col.title && typeof col.title === "string" && (
                  <span className="text-[10px] uppercase tracking-wide text-muted/70 font-semibold block mb-0.5">
                    {col.title}
                  </span>
                )}
